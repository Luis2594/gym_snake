...
if (is_numeric($dni) &&
    ...
    $clientTmp = $clientBusiness->getClientId($id);
    ...
    if (!empty($_FILES) && $_FILES["file"]["name"]) {
        ...
        foreach ($clientTmp as  $value) {
            $image = $value->getImage();
        }
    }

    $client = new Client($id, $dni, $name, $first_name, $second_name, $phone, $date_registration, $date_payment, $image);

    if ($clientBusiness->updateClient($client)) {
        header("location: ../presentation/ShowDetailClient.php?id=".$id."&action=1&msg=Cliente_creado_correctamente");
    } else {
        header("location: ../presentation/UpdateClient.php?id=" . $id . "&action=0&msg=Cliente_no_se_actualizo_correctamente");
    }
} else {
    header("location: ../presentation/UpdateClient.php?id=" . $id . "&action=0&msg=Datos_erroneos");
}
